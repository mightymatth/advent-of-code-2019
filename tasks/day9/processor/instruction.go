...
func (in *Instruction) Execute() (offset int, halt bool) {
	...
	case AdjustRelativeBaseOperation:
		return in.adjustRelativeBase(), false
	case HaltOperation:
		return 0, true
	default:
...
func (in *Instruction) storeInput() int {
	...
	in.processor.Memory[val1] = <-in.processor.Input
...
func (in *Instruction) adjustRelativeBase() int {
	val1 := in.processor.Memory[in.valueForParameter(First)]

	in.processor.RelativeBase += val1
...
func (in *Instruction) valueForMode(offset int, mode OpCodeMode) int {
	...
	case RelativeMode:
		return in.processor.RelativeBase + in.processor.Memory[in.index+offset]
	default:
